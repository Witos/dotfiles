...
while read -r line; do
    case $line in
        ...
        TIME*)
            ...
            ;;
        MAIL*)
            mail="%{F${MAIL_ICON_FG}}%{B${BAR_BG}} ${MAIL_ICON} %{F${MAIL_FG}}${line#????} "
            ;;
        MEM*)
            mem="%{F${MEM_ICON_FG}}%{B${BAR_BG}} ${MEM_ICON} %{F${MEM_FG}}${line#???} "
            ;;
        DATE*)
    ...
    echo "%{l}${wsp}${mode}%{R}%{B${BAR_BG}}${SEP_R}%{c}${music}%{r}${mem}${mail}${vol}${time}${date}"
done
